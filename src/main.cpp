...
void drawModel(const std::vector<glm::vec3> &vertices)
{
  ...
  SDL_SetRenderDrawColor(renderer, currentColor.getRed(), currentColor.getGreen(), currentColor.getBlue(), currentColor.getAlpha());
...
int main()
{
  ...
  std::string filePath = "src/StarFighter.obj";
  ...
  while (running)
  {
    ...
    angle += SPEED; // Adjust rotation speed as needed
